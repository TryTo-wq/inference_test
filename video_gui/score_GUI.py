...
def tkImage(vc,ratio):
    ...
    size = pilImage.size
    pilImage = pilImage.resize((int(size[0]*ratio),int(size[1]*ratio)),Image.ANTIALIAS)
...
class App:
    def __init__(self,window,w_win,h_win):
        ...
        print(Pic_List[Index])
        canvas.place(x = 0, y = 0.5*h_win,anchor='w')
        os.system('ffplay ' + Pic_List[Index])
        # if Mode == 'local':
        #     vc1 = cv2.VideoCapture((Pic_List[Index]))
        # elif Mode == 'internet':
        #     img = Image.open(BytesIO(requests.get(Pic_List[Index]).content)).convert('RGB')
        # size = [vc1.get(cv2.CAP_PROP_FRAME_WIDTH),vc1.get(cv2.CAP_PROP_FRAME_HEIGHT)]
...
if __name__ == '__main__':
    ...
    window = Tk()
    window.title('Quality Assessment GUI')
