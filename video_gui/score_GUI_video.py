...
class Start:
    def __init__(self,window,w_win,h_win) -> None:
        ...
        self.w_win = w_win
        self.h_win = h_win
        frame = Frame(window,width=int(0.2*w_win),height=h_win)
        self.frame = frame
        frame.pack()
        # frame.update()
        # print(frame.winfo_width())
        label = Label(frame,text='Please Enter your Name in English',font=('Times New Roman', 12))
        label.place(x = 0.1*w_win, y = 0.45*h_win,anchor='center')
        name = StringVar()
        self.name = name
        entry = Entry(frame,textvariable=name,font=('Times New Roman', 12))
        entry.place(x = 0.1*w_win, y = 0.5*h_win,anchor='center',width=100,height=40)
        entry.focus_set()
        button = Button(frame,text='Enter',command=self.save_name)
        button.place(x = 0.1*w_win, y = 0.55*h_win,anchor='center')
...
class App:
    def __init__(self,window,w_win,h_win):
        ...
        label.place(x=0.1*w_win, y=0.1*h_win, anchor='center')
        self.label_text.set("Index: {}".format(Index))

        var = DoubleVar() 
        self.var = var

        mark = Scale(frame,from_=100,  to=0,  resolution=1, orient=VERTICAL , variable=var ,length = int(h_win*0.4),showvalue=0,tickinterval=10)
        mark.place(x=0.1*w_win, y=0.5*h_win, anchor='center')
        ...
        label_scale.place(x=0.1*w_win, y=0.75*h_win, anchor='center')
        self.label_scale_text.set(round(var.get(),1))

        next_button = Button(frame,text='Next',command=self.next)
        next_button.place(x=0.1*w_win, y=0.8*h_win, anchor='center')

        pre_button = Button(frame,text='Previous',command=self.previous)
        pre_button.place(x=0.1*w_win, y=0.2*h_win, anchor='center')

        save_button = Button(frame,text='Save',command=self.save)
        save_button.place(x=0.1*w_win, y=0.9*h_win, anchor='center')
        ...
        video_path = Pic_List[Index]
        cmd = 'ffplay -autoexit -loglevel quiet {}'.format(video_path)
        print(cmd)
        os.system(cmd)
    ...
    def wheel(self,event):
        ...
        if event.delta > 0:
            ...
            self.label_scale_text.set(round(self.var.get()))
        else:
            self.var.set(self.var.get()-1)
            self.label_scale_text.set(round(self.var.get()))
    ...
    def next(self):
        ...
        if Index == len(Pic_List)-1:
            #print('No next')
            Score[str(Index)] = round(self.var.get())
            self.var.set(50)
            self.label_scale_text.set(round(self.var.get()))
            # print(Score)
...
if __name__ == '__main__':
    ...
    size_str = str(int(0.2*w_win)) + 'x' + str(h_win)
    # print(size_str)
    # add test point
